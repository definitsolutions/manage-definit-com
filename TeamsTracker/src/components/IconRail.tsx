...
export default function IconRail() {
  ...
  const activeId = 'teams'; // This app is always Teams
    ...
    <nav className="icon-rail" role="navigation" aria-label="Quick navigation">
      <div className="icon-rail-logo">
        <a href="/" title="Home" className="icon-rail-logo-link">
          <img src="/teams/logo-mark.png" alt="Definit" className="icon-rail-logo-img" />
        </a>
      </div>
      ...
      <div className="icon-rail-bottom">
        <button
          className={`icon-rail-item ${pathname?.startsWith('/teams/config') ? 'active' : ''}`}
          onClick={() => router.push('/config')}
          title="Settings"
          aria-label="Settings"
        >
          <SettingsIcon active={pathname?.startsWith('/teams/config') ?? false} />
        </button>
      </div>
    </nav>
  );
